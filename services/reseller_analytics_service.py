...
class ResellerAnalyticsService:
    ...
    def recalculate_business_analytics(self, business_user_id: str, reseller_id: str) -> Optional[BusinessUserAnalytics]:
        try:
            ...
            # Get actual remaining credits from reseller's wallet
            reseller_wallet = self.db.query(Reseller).filter(Reseller.id == reseller_id).first()
            actual_remaining = reseller_wallet.available_credits if reseller_wallet else 0
            
            # Use actual remaining credits from reseller wallet
            remaining_credits = actual_remaining
            ...
            analytics.credits_remaining = actual_remaining
